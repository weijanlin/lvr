...
"""
資料表重建腳本（含縣市代碼）
刪除現有資料表並建立新的資料表結構，包含縣市代碼和縣市名稱欄位
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('rebuild_tables_with_city.log', encoding='utf-8'),
...
def rebuild_all_tables():
    """重建所有資料庫的資料表（含縣市代碼）"""
    logger.info("🚀 開始重建所有資料表（含縣市代碼）...")
    print("🚀 開始重建所有資料表（含縣市代碼）...")
    print("=" * 80)
    ...
    if success_count == total_count:
        print("🎉 所有資料表重建完成（含縣市代碼）！")
        logger.info("🎉 所有資料表重建完成（含縣市代碼）！")
        return True
    else:
